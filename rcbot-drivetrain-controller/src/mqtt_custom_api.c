...
void mqtt_on_message_received(struct mosquitto *mosq, void *obj, const struct mosquitto_message *message){
    static struct timespec current, last={.tv_sec=0};
    // static const char* last_msg = (const char*) message->payload;
    int rc = 0;
    uint64_t delta_ms = 99999;
    char* last_msg = (char*)message->payload;
    
    clock_gettime(CLOCK_MONOTONIC_RAW, &current);
    if (last.tv_sec > 0){
        delta_ms = (last.tv_sec - current.tv_sec) * 1000 + (last.tv_nsec - current.tv_nsec) / 1000000;
    } 
    ...
    if (delta_ms > 2000) {
        ...
        if (rc != 0)
            printf("    Change direction failed\n");
    }
